...
export class ReserveTentativeSlotsUseCase {
    ...
    async execute(input: ReserveTentativeSlotsUseCaseInput):
        Promise<ReserveTentativeSlotsUseCaseOutput> {
        // スロットを仮押さえし, 予定を登録する
        const schedule =
            new Schedule(
                new ScheduleId(this.idGenerator), input.currentUserId, input.title,
                input.duration, input.place, input.selectedSlots,
